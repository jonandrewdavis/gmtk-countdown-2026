...
func _inject_event_to_viewport(viewport: SubViewport, polygon: Polygon2D, event: InputEvent) -> void:
	...
	if node is Control:
		...
		# viewport re-asserts its own cursor every frame and would override a raw
		# DisplayServer call, since the hovered Control lives inside a SubViewport.
...
func _start_animation(forward: bool):
	...
	if start_thin_R and not end_thin_R:
		...
		#tween.tween_property(visuals_container, "global_position", global_position, motion_duration)
